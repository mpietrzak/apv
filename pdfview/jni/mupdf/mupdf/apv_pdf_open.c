...
/*
 * This is a modified version of pdf_open.c file which is part of
 * MuPDF by Artifex Software, Inc.
 */
...
fz_error
pdf_loadxref_mem(pdf_xref *xref, unsigned char *bytes, unsigned int len)
{
        fz_error error;
        fz_obj *size;
        ...
        pdf_logxref("loadxref_from_memory %p\n", xref);

        buffer = fz_newbuffer(len);

        memcpy(buffer->bp, bytes, len);
        buffer->wp = buffer->bp + len;

        xref->file = fz_openrbuffer(buffer);
...
cleanup:
        fz_dropstream(xref->file);
        if (buffer) fz_dropbuffer(buffer);
        xref->file = nil;
        free(xref->table);
        xref->table = nil;
        return error;
