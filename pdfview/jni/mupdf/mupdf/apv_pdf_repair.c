...
fz_error
pdf_repairxref_fileno(pdf_xref *xref, int fileno)
{
	fz_error error;
	fz_stream *file;
	...
	error = fz_openrfileno(&file, fileno);
	if (error)
		return fz_rethrow(error, "cannot open fileno '%d'", fileno);

	pdf_logxref("repairxref_fileno '%d' %p\n", fileno, xref);
