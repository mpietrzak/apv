...
/*
 * This is a modified version of stm_read.c file which is part of MuPDF
...
fz_read(int *np, fz_stream *stm, unsigned char *mem, int n)
{
	...
	while (i < n)
	{
		...
		if (buf->rp == buf->wp)
		{
		    ...
		    /* memory buffer just shifts pointers - it doesn't produce data
             * (does not read from outside source) */
			if (stm->kind != FZ_SBUFFER) {
